<script setup lang="ts">
import AppSheet from '@/components/AppSheet.vue'
import type { ApiObject } from '@/api/butler'

defineProps<{ open: boolean; items: ApiObject[] }>()
defineEmits<{ close: []; select: [item: ApiObject] }>()
</script>

<template>
  <AppSheet :open="open" eyebrow="私有资料" title="我的资料" @close="$emit('close')">
    <view v-if="items.length" class="settings-list">
      <button
        v-for="item in items"
        :key="String(item.id)"
        class="setting-row"
        @click="$emit('select', item)"
      >
        <view
          ><text>{{ item.original_filename }}</text
          ><text>{{ item.mime_type }} · {{ item.size_bytes }} B</text></view
        >
        <text>{{
          item.knowledge_status === 'READY'
            ? '可检索'
            : item.knowledge_status === 'FAILED'
              ? '入库失败'
              : '处理中'
        }}</text>
      </button>
    </view>
    <view v-else class="note">暂无学习资料，可在聊天页上传。</view>
  </AppSheet>
</template>

<style scoped>
.settings-list {
  margin-top: 30rpx;
}
.setting-row {
  display: flex;
  width: 100%;
  min-height: 114rpx;
  align-items: center;
  justify-content: space-between;
  gap: 22rpx;
  margin: 0;
  padding: 0;
  line-height: 1.25;
  text-align: left;
  background: transparent;
  border-bottom: 1px solid #e6e9ef;
  border-radius: 0;
}
.setting-row > view {
  display: flex;
  flex: 1;
  flex-direction: column;
  gap: 8rpx;
}
.setting-row > view > text:first-child {
  font-size: 23rpx;
  font-weight: 680;
}
.setting-row > view > text:last-child {
  color: #7c8496;
  font-size: 18rpx;
}
.note {
  margin-top: 22rpx;
  padding: 20rpx;
  color: #757e91;
  font-size: 19rpx;
  line-height: 1.6;
  background: #f2f4f8;
  border-radius: 20rpx;
}
button::after {
  border: 0;
}
</style>
