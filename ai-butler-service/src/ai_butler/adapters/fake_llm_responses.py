...
def fake_intent_response(prompt: str) -> str:
    ...
    elif re.search(r"(?:制定|生成|安排).*(?:国考|省考|公务员|行测|申论|备考|计划)", value):
    ...
    needs_web = bool(
        re.search(r"(?:联网|搜索|查询|最新|今天|当前|价格|政策|公告|报名|考试时间|202\d)", value)
    ...
    return json.dumps(
        {
            ...
            "needs_web": needs_web,
            "needs_private_knowledge": attachment_count > 0
            or bool(re.search(r"(?:我的资料|附件|文件|文档)", value)),
